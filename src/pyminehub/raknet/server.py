...
class _RakNetServerProtocol(asyncio.DatagramProtocol):
    ...
    def process_UnconnectedPing(self, packet: namedtuple, addr: tuple) -> None:
        res_packet = create(ID.UnconnectedPong, packet.time_since_start, self.guid, True, self.server_id)
        self.transport.sendto(encode(res_packet), addr)

    def datagram_received(self, data: bytes, addr: tuple) -> None:
        print('{} [{}] {}'.format(addr, len(data), data.hex()))
        packet = decode(data)
